...
$(document).ready(function() {
	...
	JsVar["subsidyForm"] = new mini.Form("#subsidy_form");
});
...
function onLoadComplete(action,data) {
    ...
    JsVar["subsidyForm"].setData(data,false);
    //父窗口参数
    JsVar["data"] = data;
}
 ...
 */
function onSubmit() {
    JsVar["subsidyForm"].validate();
    if (JsVar["subsidyForm"].isValid() == false){
        return;
    }
    
    //获取表单多个控件的数据
    var dyInfo = JsVar["subsidyForm"].getData();
    ...
    getJsonDataByPost(Globals.baseActionUrl.SELECT_OPERATION_URL,param,"补贴分类名称重复校验",
	        function(result){
    			console.log(result);
    			if(result.success == systemVar.SUCCESS && result.total > 0){
    				dyInfo["success"] = systemVar.FAIL;
    				dyInfo["errormsg"] = "补贴分类名称重复";
    				closeWindow(dyInfo);
    			    ...
    			    getJsonDataByPost(Globals.baseActionUrl.ACTION_OPERATION_URL,dyInfo,"补贴分类维护",
    				        function(result){
